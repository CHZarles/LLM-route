...
class BenchMarkRunner:
    ...
    async def run(self):
        if self.request_queue is None:
            # https://github.com/xorbitsai/inference/issues/2111
    ...
    async def worker(self):
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            while self.request_left > 0:
                ...
                self.request_left -= 1
                print(
                    f"Response {len(self.requests) - self.request_left}: {json.dumps(response, ensure_ascii=False, indent=2)}"
                )
...
def main():
    ...
    tokenizer_name_or_path = "./models/chatglm-6b-int4"
    num_request = 20
    concurrency = 10
    logger.info("Preparing for benchmark.")
    # tokenizer = get_tokenizer(tokenizer_name_or_path)
    tokenizer = get_tokenizer(
        tokenizer_name_or_path, tokenizer_mode="auto", trust_remote_code=True
    )
