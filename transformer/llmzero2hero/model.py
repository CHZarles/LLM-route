...
# Define Scaled Pot Product Attention
class Attention(nn.Module):
    def __init__(self, head_size: int):
        ...
        # create a triangle matrix, which shape is context_length x context_length
    ...
    def forward(self, x):
        B, T, C = x.shape  # batch, time(context_length), channel(dim)
        ...
        # scaled dot product atttentions: Q @ K^T / sqrt(d_k)
        weights = (q @ k.transpose(1, 2)) / math.sqrt(self.head_size)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, head_size: int):
        ...
        self.heads = nn.ModuleList(
            [Attention(head_size) for _ in range(self.num_heads)]
        )
...
class TransformerLanguageModel(nn.Module):
    def __init__(self):
        ...
        # Prepare position lookup table
        self.position_encoding_lookup_table = torch.zeros(
    ...
    def forward(self, idx, targets=None):
        B, T = idx.shape
        position_embedding = self.position_encoding_lookup_table[:T, :].to(device)
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            # Crop idx to the max size of our positional embeddings tbale
            idx_crop = idx[:, -self.context_length :]
            # Get predictions
            logits, loss = self(idx_crop)
            # Get the last time step from logits where the dimensions of
            #     the logits are (B, T, C)
            logits_last_timestep = logits[:, -1, :]
            # Apply softmax to get probabilities 's distribution
            probs = F.softmax(input=logits_last_timestep, dim=-1)
            idx_next = torch.multinomial(input=probs, num_samples=1)
            # Append the new idx to the idx
...
# This function will return a batch of input and target sequences
...
def estimate_loss():
    ...
    for split in ["train", "valid"]:
        ...
        for k in range(eval_iters):
            infer, targets = get_batch(split)
            logits, loss = model(infer, targets)
            losses[k] = loss.item()
